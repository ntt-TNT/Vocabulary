...
import com.example.vocabulary.Bean.TranslateBean;
import com.example.vocabulary.MyApplication;
...
public class JinshanParseUtil {
    ...
    public static void parseJinshanChineseToEnglishJSONWithGson(String result) {
        ...
        try {
            Gson gson = new Gson();
            TranslateBean translate = gson.fromJson(result, TranslateBean.class);
            ...
            queryText = translate.getWordName();

            for (TranslateBean.SymbolsBean voiceMsg : translate.getSymbols()){
                voiceText += voiceMsg.getWordSymbol()+" , ";
                voiceUrlText += voiceMsg.getSymbolMp3();
                for(TranslateBean.SymbolsBean.PartsBean meanMsg : voiceMsg.getParts()){
                    meanText += meanMsg.getPartName()+": ";
                    for(TranslateBean.SymbolsBean.PartsBean.MeansBean mean :meanMsg.getMeans()){
                        meanText += mean.getWordMean() + "; ";
                    }
